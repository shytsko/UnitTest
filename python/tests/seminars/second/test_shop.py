...
def store_items():
    ...
    product_names = ["bacon", "beef", "ham", "salmon", "carrot", "potato", "onion", "apple", "melon", "rice", "eggs",
                     "yogurt"]
    product_price = [Decimal(170.00), Decimal(250.00), Decimal(200.00), Decimal(150.00), Decimal(15.00), Decimal(0.00),
                     Decimal(20.00), Decimal(59.00), Decimal(88.00), Decimal(100.00), Decimal(80.00), Decimal(55.00)]
...
def test_price_cart_is_correct_calculated(cart):
...
# <br><b>Ожидаемый результат:</b>
# Стоимость корзины посчиталась корректно
def test_price_cart_products_same_type_is_correct_calculated(cart):
...
def test_when_changing_cart_cost_recalculation_is_called(cart):
    add_products_pk = 1, 5, 8, 11, 5, 11, 11
...
def test_quantity_products_store_changing(cart):
    add_product_pk = 8
    add_quantity = 3
...
def test_products_disappear_from_store(cart):
    add_product_pk = 8
    extra_attempts = 2
...
def test_deleted_product_is_returned_to_shop(cart):
    add_product_pk = 8
    add_quantity = 5
    remove_quantity = 3
...
def test_incorrect_product_selection_causes_exception(cart, incorrect_add_product_pk):
...
def test_price_cart_is_correct_calculated_ext(cart):
    add_product_pk = 8
    cart.add_product(add_product_pk)
